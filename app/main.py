"""Точка входа FastAPI после уборки ML-кода с прошлых недель.
...
def health() -> dict[str, str]:
    """Простой liveness-пробник для CI и автодеплоя."""
    return {"status": "ok"}
...
with gr.Blocks(title="ML Q&A", fill_height=True) as demo:
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            with gr.Row():
                msg = gr.Textbox(placeholder="Спросите что-нибудь…", scale=4, show_label=False)
